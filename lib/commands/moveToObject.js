...
module.exports = function moveToObject(selector, xoffset, yoffset) {
    ...
    return this.element(selector).then(function(res) {
        return this.elementIdSize(res.value.ELEMENT).then(function(size) {
            return this.elementIdLocation(res.value.ELEMENT).then(function(location) {
                return {
                    element: res.value,
                    size: size,
                    location: location
        ...
        if(hasOffsetParams) {
            ...
            x = res.size.value.width / 2;
            y = res.size.value.height / 2;
        }

        if (this.isMobile) {
            return this.touchMove(x, y);
        } else {
            return this.moveTo(res.element.ELEMENT, x, y);
        }
    })
